...
每个插件都位于其自己的目录中，该目录包含清单和你的 skills、agents 或 hooks。现在创建一个：报告错误代码
复制
询问AI
...
位于 `.claude-plugin/plugin.json` 的清单文件定义了你的插件的身份：其名称、描述和版本。Claude Code 使用此元数据在插件管理器中显示你的插件。在你的插件文件夹内创建 `.claude-plugin` 目录：报告错误代码
复制
询问AI
...
然后使用以下内容创建 `my-first-plugin/.claude-plugin/plugin.json`：my-first-plugin/.claude-plugin/plugin.json
报告错误代码
复制
询问AI
`{
...
Skills 位于 `skills/` 目录中。每个 skill 是一个包含 `SKILL.md` 文件的文件夹。文件夹名称成为 skill 名称，以插件的命名空间为前缀（在名为 `my-first-plugin` 的插件中的 `hello/` 创建 `/my-first-plugin:hello`）。在你的插件文件夹中创建一个 skill 目录：报告错误代码
复制
询问AI
...
然后使用以下内容创建 `my-first-plugin/skills/hello/SKILL.md`：my-first-plugin/skills/hello/SKILL.md
报告错误代码
复制
询问AI
`---
...
使用 `--plugin-dir` 标志运行 Claude Code 以加载你的插件：报告错误代码
复制
询问AI
...
Claude Code 启动后，尝试你的新命令：报告错误代码
复制
询问AI
...
通过接受用户输入使你的 skill 动态化。`$ARGUMENTS` 占位符捕获用户在 skill 名称后提供的任何文本。更新你的 `hello.md` 文件：my-first-plugin/commands/hello.md
报告错误代码
复制
询问AI
`---
...
重启 Claude Code 以获取更改，然后尝试使用你的名字运行命令：报告错误代码
复制
询问AI
...
在 `my-plugin/.claude-plugin/plugin.json` 处创建清单文件：my-plugin/.claude-plugin/plugin.json
报告错误代码
复制
询问AI
`{
...
如果你在设置中有 hooks，请创建一个 hooks 目录：报告错误代码
复制
询问AI
...
使用你的 hooks 配置创建 `my-plugin/hooks/hooks.json`。从你的 `.claude/settings.json` 或 `settings.local.json` 复制 `hooks` 对象，因为格式相同。命令在 stdin 上接收 hook 输入作为 JSON，所以使用 `jq` 提取文件路径：my-plugin/hooks/hooks.json
报告错误代码
复制
询问AI
`{
