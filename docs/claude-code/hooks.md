...
[CLI 参考
](/docs/zh-CN/cli-reference)[交互模式
](/docs/zh-CN/interactive-mode)[checkpointing
](/docs/zh-CN/checkpointing)[Hooks 参考
](/docs/zh-CN/hooks)[插件参考
](/docs/zh-CN/plugins-reference)

有关包含示例的快速入门指南，请参阅 [Claude Code hooks 入门](/docs/zh-CN/hooks-guide)。


## [​
](#配置)配置

Claude Code hooks 在您的 [设置文件](/docs/zh-CN/settings) 中配置：
...
企业管理员可以使用 `allowManagedHooksOnly` 来阻止用户、项目和插件 hooks。请参阅 [Hook 配置](/docs/zh-CN/settings#hook-configuration)。
...
[插件](/docs/zh-CN/plugins) 可以提供与您的用户和项目 hooks 无缝集成的 hooks。启用插件时，插件 hooks 会自动与您的配置合并。
**插件 hooks 的工作原理**：
...
### [​
](#skills、agents-和-slash-commands-中的-hooks)Skills、Agents 和 Slash Commands 中的 Hooks

除了设置文件和插件外，hooks 还可以直接在 [Skills](/docs/zh-CN/skills)、[subagents](/docs/zh-CN/sub-agents) 和 [slash commands](/docs/zh-CN/slash-commands) 中使用 frontmatter 定义。这些 hooks 的作用域限于组件的生命周期，仅在该组件处于活动状态时运行。
...
### [​
](#基于提示的-hooks-如何工作)基于提示的 hooks 如何工作

基于提示的 hooks 不是执行 bash 命令，而是：


- 将 hook 输入和您的提示发送到快速 LLM（Haiku）

- LLM 使用包含决策的结构化 JSON 进行响应
...
### [​
](#示例：智能-stop-hook)示例：智能 Stop hook
...
### [​
](#示例：带有自定义逻辑的-subagentstop)示例：带有自定义逻辑的 SubagentStop
...
### [​
](#与-bash-命令-hooks-的比较)与 bash 命令 hooks 的比较
...
- `Task` - Subagent 任务（请参阅 [subagents 文档](/docs/zh-CN/sub-agents)）
...
### [​
](#notification-输入)Notification 输入
...
### [​
](#userpromptsubmit-输入)UserPromptSubmit 输入
...
### [​
](#stop-和-subagentstop-输入)Stop 和 SubagentStop 输入
...
### [​
](#sessionstart-输入)SessionStart 输入
...
#### [​
](#pretooluse-决策控制)`PreToolUse` 决策控制
...
#### [​
](#permissionrequest-决策控制)`PermissionRequest` 决策控制
...
#### [​
](#posttooluse-决策控制)`PostToolUse` 决策控制
...
#### [​
](#userpromptsubmit-决策控制)`UserPromptSubmit` 决策控制

`UserPromptSubmit` hooks 可以控制是否处理用户提示并添加上下文。
**添加上下文（退出代码 0）**：
有两种方式向对话添加上下文：

**纯文本 stdout**（更简单）：写入 stdout 的任何非 JSON 文本都被添加为上下文。这是注入信息的最简单方式。


**带有 `additionalContext` 的 JSON**（结构化）：使用下面的 JSON 格式以获得更多控制。`additionalContext` 字段被添加为上下文。
...
#### [​
](#stop/subagentstop-决策控制)`Stop`/`SubagentStop` 决策控制
...
#### [​
](#sessionstart-决策控制)`SessionStart` 决策控制
...
#### [​
](#sessionend-决策控制)`SessionEnd` 决策控制

`SessionEnd` hooks 在会话结束时运行。它们无法阻止会话终止，但可以执行清理任务。

#### [​
](#退出代码示例：bash-命令验证)退出代码示例：Bash 命令验证
...
#### [​
](#json-输出示例：userpromptsubmit-以添加上下文和验证)JSON 输出示例：UserPromptSubmit 以添加上下文和验证
...
#### [​
](#json-输出示例：带有批准的-pretooluse)JSON 输出示例：带有批准的 PreToolUse
...
Claude Code hooks 与 [Model Context Protocol (MCP) 工具](/docs/zh-CN/mcp) 无缝协作。当 MCP 服务器提供工具时，它们以特殊命名模式出现，您可以在 hooks 中匹配。
...
### [​
](#为-mcp-工具配置-hooks)为 MCP 工具配置 Hooks
...
有关包括代码格式化、通知和文件保护的实际示例，请参阅入门指南中的 [更多示例](/docs/zh-CN/hooks-guide#more-examples)。
...
- **测试边界情况** - 尝试使用不寻常的文件路径或输入的 hooks

- **监控系统资源** - 在 hook 执行期间检查资源耗尽

- **使用结构化日志** - 在您的 hook 脚本中实现日志记录
...
[checkpointing](/docs/zh-CN/checkpointing)[插件参考](/docs/zh-CN/plugins-reference)
