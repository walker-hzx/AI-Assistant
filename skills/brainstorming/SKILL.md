---
name: brainstorming
description: "在任何创意工作之前必须使用 - 创建功能、构建组件、添加功能或修改行为。在实施之前探索用户意图、需求和设计。"
model: sonnet
user-invocable: true
---

# 头脑风暴：将想法转化为设计

## 概述

通过自然的协作对话，帮助将想法转化为完整的设计和规格说明。

首先了解当前项目的上下文，然后一次提出一个问题来完善想法。一旦理解了要构建的内容，呈现设计并获得用户批准。

<HARD-GATE>
在呈现设计并获得用户批准之前，不要调用任何实现 skill、编写任何代码、搭建任何项目或采取任何实现行动。这适用于每个项目，无论其看起来多么简单。
</HARD-GATE>

## 反模式："这太简单了，不需要设计"

每个项目都要经历这个过程。一个待办列表、一个单功能工具、一个配置更改——所有这些都要。"简单"的项目正是未经审视的假设造成最多浪费工作的地方。设计可以很简短（真正简单的项目几句话即可），但你必须呈现它并获得批准。

## 检查清单

你必须为以下每个项目创建任务并按顺序完成：

1. **检查需求文档** — 扫描 `docs/requirements/` 目录，避免重复讨论已有需求
2. **探索项目上下文** — 检查文件、文档、最近的提交
3. **【重要】需求完整性检查** — 必须通过问答确认所有关键维度
4. **提出澄清问题** — 一次一个，理解目的/约束/成功标准
5. **提出 2-3 种方案** — 附带权衡取舍和你的推荐
6. **呈现设计** — 根据复杂程度分章节展示，每章后获得用户批准
7. **编写设计文档** — 保存到 `docs/plans/YYYY-MM-DD-<topic>-design.md` 并提交

## 进度追踪（文件式规划）

**在开始头脑风暴时，同步创建进度追踪文件**：

1. **创建进度文件** `docs/plans/progress.md`（如果不存在）：
```markdown
# 项目进度追踪

## 当前任务

| 任务 | 阶段 | 状态 | 更新时间 |
|------|------|------|----------|
| [任务名] | brainstorming | 🔄 进行中 | YYYY-MM-DD |

## 最近活动

| 时间 | 活动 | 说明 |
|------|------|------|
| YYYY-MM-DD HH:MM | 开始 | 开始头脑风暴：xxx |
```

2. **每个阶段完成后更新进度**：
- 更新任务状态（pending → in_progress → complete）
- 记录完成时间和活动
- 添加下一步计划

3. **文件位置**：`docs/plans/progress.md`

## 【重要】讨论草稿文档

**创建草稿文档**：

在开始头脑风暴时，创建草稿文档记录讨论过程：

```markdown
# [主题] 头脑风暴草稿

> 创建时间：YYYY-MM-DD HH:MM

## 讨论记录

### Q1: [问题]
**A**: [用户回答]

### Q2: [问题]
**A**: [用户回答]

### ...

## 关键要点

- [要点1]
- [要点2]

## 待确认

- [ ] 问题1
- [ ] 问题2

## 想法碎片

- [想法1]
- [想法2]
```

**实时更新**：

在讨论过程中，每次对话后更新草稿：
1. 记录问题和回答
2. 提取关键要点
3. 标记待确认事项
4. 记录想法碎片

**文件位置**：`docs/plans/YYYY-MM-DD-<topic>-draft.md`

**为什么需要**：
- 讨论时间长不会遗忘
- 方便回顾之前讨论的内容
- 最后整理成设计文档时有参考

## 流程

**【重要】深度分析需求：**
在开始分析用户需求时，不要只停留在表面理解，而是要从更高维度分析：

1. **这是真正的核心问题吗？**
   - 用户说的具体问题是什么
   - 真正要解决的是什么

2. **从多维度分析：**
   - **技术维度**：技术方案是否合理
   - **价值维度**：解决这个问题价值有多大
   - **架构维度**：对项目整体有什么影响

3. **分析后再开始：**
   - 先和用户确认理解是否正确
   - 如果发现更深层的问题，先讨论清楚
   - 不要急着进入细节

**开始前 - 检查需求文档：**
- 扫描 `docs/requirements/` 目录
- 如果发现与当前主题相关的已有需求，提醒用户
- 确认是新建需求还是复用/扩展已有需求
- 避免重复讨论已确认的需求

**【重要】需求完整性检查（必须全部确认）**

在提出澄清问题之前，必须先完成需求完整性检查。这是强制步骤，**所有项确认后才能进入方案设计**。

### 需求完整性检查清单

| 维度 | 关键问题 | 用户回答 | 确认 |
|------|---------|---------|------|
| **核心功能** | 1. 这个功能要解决什么问题？ | | [ ] |
| | 2. 用户如何使用这个功能？ | | [ ] |
| | 3. 有哪些子功能/操作？ | | [ ] |
| **边界条件** | 4. 输入数据有什么限制？（长度、格式、范围） | | [ ] |
| | 5. 数据量有多大？（列表条数、并发量） | | [ ] |
| | 6. 有什么性能要求？ | | [ ] |
| **异常处理** | 7. 出错时应该怎么处理？（提示、回滚、重试） | | [ ] |
| | 8. 失败如何恢复？ | | [ ] |
| | 9. 有哪些边界情况需要考虑？ | | [ ] |
| **依赖关系** | 10. 依赖哪些现有模块/功能？ | | [ ] |
| | 11. 对数据库/接口有什么要求？ | | [ ] |
| | 12. 是否需要外部服务/第三方库？ | | [ ] |

### 检查流程

1. **逐个维度提问**：一次问一个维度的问题（核心功能 → 边界条件 → 异常处理 → 依赖关系）
2. **记录用户回答**：在草稿文档中记录每个问题的回答
3. **确认完整性**：所有问题都有明确回答后，标记为确认
4. **强制门控**：**有任何一项未确认，不得进入方案设计阶段**

### 阶段 2：【必须】需求自检清单

**问答完成后，逐项自检**，确保没有遗漏：

```
## 需求自检清单

请逐项确认以下每个方面：

### 1. 功能完整性自检
- [ ] 核心功能都已列出
- [ ] 每个功能有明确的操作步骤
- [ ] 列表类功能有分页考虑
- [ ] 编辑类功能有数据校验考虑

### 2. 边界条件自检
- [ ] 空值/空数据情况已考虑
- [ ] 最大数量/长度限制已明确
- [ ] 特殊字符/格式已考虑
- [ ] 并发/大数据量已考虑

### 3. 异常处理自检
- [ ] 网络异常情况已考虑
- [ ] 接口错误有明确处理方式
- [ ] 权限不足有明确提示
- [ ] 操作失败有明确反馈

### 4. 用户体验自检
- [ ] 加载状态有提示
- [ ] 操作有反馈（成功/失败）
- [ ] 关键操作有确认提示

### 5. 验收标准自检
- [ ] 成功标准已明确（什么算完成）
- [ ] 失败标准已明确（什么算失败）
- [ ] 可测试的验收条件已列出

### 6. 风险自检
- [ ] 技术风险已识别
- [ ] 依赖风险已识别
- [ ] 已有缓解措施
```

### 自检流程

1. **展示自检清单**：将清单展示给用户
2. **逐项确认**：请用户逐项确认"有没有考虑"
3. **补充遗漏**：如果用户说有遗漏，补充提问
4. **标记完成**：所有项确认后，标记为需求完成
5. **进入设计**：确认后才能进入方案设计阶段

### 常见问题处理

**用户说"不知道"或"后面再说"**：
- 说明这个方面不确认的风险
- 提供行业常规做法让用户选择
- 如果实在无法确认，标记为"待确认"并在设计中标注

**用户跳过某个问题**：
- 必须回到该问题，不能直接进入下一步
- 可以说："为了确保设计完整，这项需要确认一下"

**用户自检时说"应该有了"**：
- 追问具体场景，确保真的考虑到了
- 不要假设用户已经想到

### 常见问题处理

**如果用户回答"不知道"或"后面再说"：**
- 说明这个问题的重要性
- 提供可能的选项让用户选择
- 如果实在无法确认，标记为"待后续确认"，并在设计中标注风险

**如果用户跳过某个问题：**
- 必须回到该问题，不能直接进入下一步
- 可以说："为了确保设计完整，还需要确认一下 [问题]"

---

**理解想法：**
- 首先检查当前项目状态（文件、文档、最近提交）
- 完成需求完整性检查（强制步骤）
- 一次提一个问题来完善想法
- 尽可能选择封闭式问题，开放式也可以
- 每条消息一个问题——如果一个主题需要更多探索，分成多个问题
- 重点理解：目的、约束、成功标准

**功能范围确认（在探索想法后、呈现方案前）：**
- 明确功能模块：从用户描述中提取实体（模块）
- 明确操作类型：每个模块需要哪些操作（列表、新增、编辑、删除等）
- 记录到设计文档的"功能范围"章节
- 注意：细节补充在 writing-plans 阶段完成

**探索方案：**
- 提出 2-3 种不同方案，附带权衡取舍
- 会话性地呈现选项，说明你的推荐和理由
- 先说出推荐选项并解释原因

**呈现设计：**
- 一旦相信理解了要构建的内容，就呈现设计
- 根据复杂程度调整每个部分：简单的几句话，复杂的多达 200-300 字
- 每个部分后询问是否正确
- 涵盖：架构、组件、数据流、错误处理、测试
- 当事情不对劲时，准备好回去澄清

## 设计文档模板

```markdown
# [功能名称] 设计文档

## 概述
[2-3 句话总结]

## 功能范围

| 模块 | 功能点 | 优先级 |
|------|--------|--------|
| [模块名] | 列表查看 | P0 |
| [模块名] | 新增 | P0 |
| [模块名] | 编辑 | P0 |
| [模块名] | 删除 | P0 |
| [模块名] | 批量操作 | P1 |
| [模块名] | 导入/导出 | P2 |

**常见功能点参考：**
- P0（必须）：列表、新增、编辑、删除、详情
- P1（建议）：搜索、分页、批量删除、状态变更
- P2（可选）：导入导出、审计日志、统计分析

## 背景
- 问题描述：
- 期望结果：
- 成功标准：

## 方案选择

### 方案 A: [名称]
- 优点：
- 缺点：
- 适用场景：

### 方案 B: [名称]
- 优点：
- 缺点：
- 适用场景：

### 推荐方案
[说明为什么推荐这个方案]

## 架构设计

### 数据模型
[数据库表结构]

### API 设计
[API 端点]

### 组件设计
[前端组件]

## 用户流程

1. [步骤 1]
2. [步骤 2]
3. [步骤 3]

## 错误处理
[异常情况]

## 测试策略
- 单元测试：
- 集成测试：
- E2E 测试：

## 风险与缓解
- **风险**：[描述]
  - 缓解措施：

## 验收标准
- [ ] 标准 1
- [ ] 标准 2
```

## 与需求讨论的关系

| Skill | 职责 | 使用时机 |
|-------|------|----------|
| **discuss-requirements** | 需求讨论 + 实时记录 | 需求不明确时 |
| **brainstorming** | 需求转化为设计 | 需求明确后 |

**流程**：
1. 需求不明确 → 使用 discuss-requirements
2. 需求明确 → 使用 brainstorming
3. 设计完成 → 使用 writing-plans

## 之后做什么

**文档：**
- 将验证后的设计写入 `docs/plans/YYYY-MM-DD-<topic>-design.md`
- **【重要】设计文档必须包含需求自检确认**：
  - 在文档头部添加"需求自检确认"章节
  - 列出哪些项已确认、哪些待确认
  - 标记为"✅ 已确认需求"或"⚠️ 待确认"
- **更新总体需求文档** `docs/requirements/README.md`：
  - 在需求列表中添加新需求
  - 更新关联计划链接
- 提交设计文档

**【重要】设计文档输出格式要求**

```markdown
# [功能名称] 设计文档

## 需求自检确认

| 检查项 | 状态 | 备注 |
|--------|------|------|
| 功能完整性 | ✅ 已确认 | 核心功能已列出 |
| 边界条件 | ✅ 已确认 | 已明确限制 |
| 异常处理 | ⚠️ 待确认 | 接口错误处理待定 |
| 验收标准 | ✅ 已确认 | 成功/失败标准已明确 |
| ... | ... | ... |

> **重要**：writing-plans 制定计划时，必须对照此自检确认表
</markdown>

**实施：**
- 调用 writing-plans skill 创建详细实施计划
- 不要调用任何其他 skill。writing-plans 是的下一步。

## 【重要】维护总体需求文档

每次需求有变更时，必须更新 `docs/requirements/README.md`：

### 新增需求

1. 在需求列表中添加新条目：
```markdown
| 序号 | 需求名称 | 状态 | 创建时间 | 最后更新 | 关联计划 |
|------|----------|------|----------|----------|----------|
| N | [需求名称] | 📋 规划中 | YYYY-MM-DD | - | [链接] |
```

2. 在需求索引中添加链接：
```markdown
- [需求名称](./YYYY-MM-DD-需求名称-需求.md)
```

### 调整需求

1. 更新需求列表中的状态和时间
2. 在需求索引中保留链接（指向最新版本）

### 完成需求

1. 将状态更新为 ✅ 已完成
2. 更新最后更新时间

### 【重要】作废过时需求

当讨论新需求时，如果发现某个老需求已经不需要实现，必须将其标记为"已作废"：

1. **检查是否有需要作废的需求**：
   - 架构调整后不再需要的功能
   - 被新需求替代的功能
   - 用户明确表示不需要的功能

2. **更新 README.md**：
   - 在"当前确认需求"中移除
   - 添加到"已作废需求"部分

```markdown
## 已作废需求（不实现，仅记录）

| 序号 | 需求名称 | 作废原因 | 替代方案 |
|------|----------|---------|----------|
| 1 | 场景管理功能 | 架构调整，新架构通过代码/配置定义 | 场景配置化定义 |
```

3. **告知用户**：
   - "已标记需求A为已作废，原因是..."
   - "如果需要恢复，请告知"

4. **与 writing-plans 关联**：
   - writing-plans 制定计划时会自动忽略已作废需求
   - 用户不需要手动排除

## 【重要】代码资产复用意识

在设计和实施过程中，涉及代码复用时：

### 决策时机

| 阶段 | 做什么 | 产出 |
|------|--------|------|
| brainstorming | 概念确认 | 确认是否需要新建 |
| writing-plans | 任务分配 | 标注可复用代码 |
| executing-plans | 生成代码 | 具体复用实现 |

### 查找方式

- 用 Glob 扫描目录
- 用 Grep 搜索关键词
- 读取组件代码评估能力

### 评估原则

- **能用则用**：不要重复造轮子
- **不能则换**：不能满足需求的不勉强
- **扩展优先**：优先扩展现有代码
- **诚实评估**：不确定的先问用户

---

## 关键原则

- **一次一个问题** - 不要用多个问题淹没用户
- **优先封闭式问题** - 比开放式更容易回答
- **YAGNI 原则** - 从所有设计中删除不必要的功能
- **探索替代方案** - 在确定之前总是提出 2-3 种方案
- **增量验证** - 呈现设计，获得批准后再继续
- **保持灵活** - 当事情不对劲时回去澄清
