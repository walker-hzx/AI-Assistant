...
1. **读取需求总览文件**：`docs/requirements/README.md`
2. **这是唯一权威来源**：制定计划时只基于"当前确认需求"部分
3. **忽略已作废需求**：不要基于"已作废需求"部分制定计划
4. **如果文件不存在或为空**：停止制定计划，告知用户需要先确认需求

### 步骤 2：【强制】对照需求确认

**必须对照 brainstorming 产出的设计文档检查**：

1. **读取设计文档**：`docs/plans/YYYY-MM-DD-<topic>-design.md`
2. **检查需求自检确认表**：
   - 每个"已确认"的需求是否都有对应任务？
   - 是否有"待确认"的需求需要特别处理？
3. **功能覆盖检查**：
   - 设计文档中的每个功能模块是否都有任务覆盖？
   - 是否有功能遗漏？
4. **如果有遗漏**：
   - 标记为遗漏项
   - 在计划中补充任务
...
1. 任务数量 > 5 个
2. 涉及多个模块/组件
3. 有多个阶段（Phase 1, Phase 2...）
4. 涉及数据库变更
5. 涉及 API 接口变更
6. 用户明确要求确认

**跳过确认的后果**：计划可能遗漏关键步骤，导致执行时返工。
...
> **【新增】界面/组件设计任务**（涉及前端功能时必须包含）：
>
> 前端项目在任务列表中需要包含界面设计相关任务：
> ```markdown
> ### 任务 N：界面/组件设计
>
> **界面设计**：
...
> ```
>
> **说明**：这是实现层的界面设计，只关注"怎么做界面"，不关注用户流程（用户流程在 brainstorming 阶段已确认）
...
下一步：executing-plans（执行计划）
```

> **简化版**（如果计划简单明确）：
> "计划已整理好，包含 N 个任务，可以开始执行了吗？下一步是 executing-plans"
