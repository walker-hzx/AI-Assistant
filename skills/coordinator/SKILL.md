...
```
1. 接收用户需求，理解需求内容
2. 【思考1】调用 thinking-coach 分析任务类型和复杂度：
   - 这是什么类型的任务？（功能开发/Bug修复/调研/分析等）
   - 任务复杂度如何？（简单/中等/复杂）
   - 有什么潜在风险？
3. 根据需求内容判断任务类型，记录到调度记录
4. 使用 Write 工具创建调度记录：`docs/coordinator/<task>-coordinator.md`
5. 填写任务基本信息（时间、任务、类型）
6. 初始化阶段状态（全部设为"待执行"）
7. 更新状态为"进行中"
...
| "优化UI"、"优化UX"、"页面丑" | UI/UX 优化 | ui-ux-reviewer + executor |
...
【执行轮次 N】
1. 从计划文档中获取当前轮次的任务
2. 根据任务上下文选择合适的 Subagent（见子步骤 4.1）
3. 调度 Subagent 执行
4. 等待 Subagent 完成
5. 读取 Subagent 输出文档
6. 检查执行结果
...
| ui-ux-reviewer | docs/execution/, docs/plans/ | docs/reviews/ | 读取代码和执行结果，输出 UI/UX 优化建议 |
...
【管家】检查审查结果 → 通过
    ↓
【管家】质量评估 → 生成 docs/quality/...summary.md
    ↓
【管家】更新调度记录 → 已完成
    ↓
【管家】任务完成
